...
const addProduct = asyncHandler(async (req, res) => {
    ...
    if (
        [
            ...
            productPrice,
            productAddress,
        ].some((fields) => !fields || fields.trim() === "")
    ) {
    ...
    const product = await Products.findByPk(
        productAddInDb.dataValues.product_id,
        { raw: true },
    );
    ...
    return res
        .status(201)
        .json(new ApiResponse(201, product, "Product successfully added"));
});
const editProduct = asyncHandler(async (req, res) => {
    ...
    if (
        [productType, productName, productDetails, productAddress].some(
    ...
    if (!productImageLocalPath) {
        ...
        return res
            .status(201)
            .json(new ApiResponse(201, product, "Product updated"));
    }
    ...
    return res.status(201).json(new ApiResponse(201, product, "Product updated"));
});
const removeProduct = asyncHandler(async (req, res) => {
    ...
    return res.status(201).json(new ApiResponse(201, "Product deleted"));
});
...
const getAllUser = asyncHandler(async (req, res) => {
    ...
    return res
        .status(201)
        .json(new ApiResponse(201, { users, countingData }, "All users fetched"));
});
...
const getAllOrders = asyncHandler(async (req, res) => {
    ...
    return res
        .status(201)
        .json(new ApiResponse(201, orders, "All orders fetched"));
});
...
const getAllBill = asyncHandler(async (req, res) => {
    ...
    if (!bill) {
        throw new ApiError(404, "Orders bill no found with this order_item_id");
    }
    console.log("AB");
    return res.status(201).json(new ApiResponse(201, bill, "All bill fetched"));
});
const operationalHighlight = asyncHandler(async (req, res) => {
    ...
    try {
        const [
            ...
            Order_Bill.sum("totalPrice", {
                where: { bill_date: { [Op.gte]: startOfMonth } },
            }),
        ]);
        ...
        throw new ApiError(
            500,
            "Something went wrong while we get data from the database",
            [error.message],
        );
    }
});
const graphData = asyncHandler(async (req, res) => {
    ...
    const bill = await Order_Bill.findAll({
        attributes: ["totalPrice", "bill_date"],
        raw: true,
    });
    ...
    return res.status(201).json(new ApiResponse(201, result, "ok"));
});
